...
namespace DL.Entity
{
    [MetadataType(typeof(WardDetailMD))]
    public partial class WardDetail
    {
        #region --- Property Declaration ---
        public class WardDetailMD
        {
        ...
        public LookupModel GetById(Guid id)
        {
            try
            {
                ...
                using (var db = new HMSEntities())
                {
                    WardDetail entity = db.WardDetails.Find(id);
                    if (entity != null)
                    {
                        model.Id = entity.Id;
        ...
        public LookupModel GetByName(string name)
        {
            try
            {
                ...
                using (var db = new HMSEntities())
                {
                    WardDetail entity = db.WardDetails.FirstOrDefault(t => t.Name == name);
                    if (entity != null)
                    {
                        model.Id = entity.Id;
                        model.Name = entity.Name;
        ...
        public bool Create(LookupModel model)
        {
            try
            {
                ...
                if (!IsExist(model))
                {
                    using (var db = new HMSEntities())
                    {
                        WardDetail entity = new WardDetail();
        ...
        public bool Update(LookupModel model)
        {
            try
            {
                ...
                if (!IsExist(model))
                {
                    using (var db = new HMSEntities())
                    {
                        WardDetail entity = db.WardDetails.Find(model.Id);
        ...
        private bool IsExist(LookupModel model)
        {
            try
            {
                Expression<Func<WardDetail, bool>> predicate = null;
                ...
                using (var db = new HMSEntities())
                {
                    ...
                    if (db.WardDetails.Any(predicate))
                    {
                        return true;
                    }
                    else
